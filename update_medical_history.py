...
class UpdateMedicalHistoryWindow:
    def __init__(self, db_connection, patient_id):
        self.db_connection = db_connection
        self.patient_id = patient_id

        self.window = tk.Toplevel()
        self.window.title("Update Medical History")
    ...
    def create_widgets(self):
        ...
        # Labels
        labels = ["Diagnosis", "Prescription", "Visit Date"]
        self.entry_vars = []
    ...
    def fetch_existing_details(self):
        try:
            ...
            query = "SELECT diagnosis, prescription, visit_date FROM medical_history WHERE id = %s"
    ...
    def update_details(self):
        ...
        try:
            cursor = self.db_connection.cursor()
            query = "UPDATE medical_history SET diagnosis = %s, prescription = %s, visit_date = %s WHERE id = %s"
            cursor.execute(query, (diagnosis, prescription,
                            visit_date, self.patient_id))
            ...
            messagebox.showerror(
                "Error", f"Error updating medical history: {err}")
...
# Assuming patient_id is the ID of the patient to be updated
# patient_id = 1  # Replace with the actual patient ID
# update_window = UpdateMedicalHistoryWindow(db_connection, patient_id)
# update_window.window.mainloop()
